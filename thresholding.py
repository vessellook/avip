...
def transform_otsu(grayscale: Union[Image.Image, np.ndarray]) -> np.ndarray:
    np_img = np.asarray(grayscale, dtype=np.uint8)
    min_val = np_img.min()
    max_val = np_img.max() + 1
    hist, edges = np.histogram(np_img, density=True, bins=np.arange(min_val, max_val))
...
def transform_niblack(grayscale: np.ndarray, blockSize: int, k: float = -0.2) -> np.ndarray:
    result = np.zeros_like(grayscale)
    niBlackThreshold(grayscale, 1, cv.THRESH_BINARY, blockSize, k, result)
    return result
